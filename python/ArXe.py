...
file_conus = outdir + 'chi2_nsi_2DAr.dat'  # Datos (Ar)
config_file = outdir + 'nsi_configAr.txt'       # Etiquetas del caso actual
...
plt.title(f'Comparación de Sensibilidad NSI (90% C.L.)\nLXe vs LAr (RED-100)', fontsize=12)
plt.grid(True, linestyle=':', alpha=0.4)

# Leyenda manual
legend_elements = [
    Line2D([0], [0], color='#1f77b4', lw=2, label='RED-100 (Xe)'),
    Line2D([0], [0], color='red', lw=2, linestyle='--', label='RED-100 (Ar)')
]
plt.legend(handles=legend_elements, loc='upper right')

plt.tight_layout()
plt.savefig(outdir + 'comparacion_nsi_Ar_Xe.png', dpi=300)
plt.show()
