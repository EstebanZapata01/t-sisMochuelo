...
module mod_tnr_to_e
  ...
  real(dp), allocatable, dimension(:) :: T_dense, Qy_dense
...
contains
  ...
  subroutine inicializar_nest()
    integer :: u_in, iostat, i
    character(len=500) :: linea
    real(dp) :: T_temp, Qy_temp
    ...
    allocate(T_dense(n_puntos_actual), Qy_dense(n_puntos_actual))

    ! ---- 2ª pasada: leer los datos ----
    i = 0
    do
       read(u_in, '(A)', iostat=iostat) linea
       if (iostat /= 0) exit
