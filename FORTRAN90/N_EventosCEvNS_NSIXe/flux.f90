...
module flux
  ...
  ! Variables para normalización expuestas para el programa principal
...
contains
  ...
  ! Esta función obliga a calcular la integral la primera vez que se llama
  function flujo_diferencial(E_nu) result(flujo)
    ...
    if (.not. norm_initialized) then
       ...
       write(*,*) '=== DIAGNOSTICO DEL ESPECTRO (FLUJO EXPLICITO) ==='
       write(*,*) 'Flujo Total Asumido (phi): ', phi_total
       write(*,*) 'Integral del espectro    : ', spectrum_integral
       write(*,*) '=================================================='
    end if
